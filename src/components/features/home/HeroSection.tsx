...
const CONTENT: Record<string, HeroContent> = {
  GUEST: {
    badge: {
      icon: <Star size={14} className='fill-indigo-400 text-indigo-400' />,
    ...
    ctas: [
      { label: 'Find a Tutor', href: '/tutors', icon: <Search size={18} />, variant: 'primary' },
      { label: 'Become a Tutor', href: '/register', icon: <Users size={18} />, variant: 'outline' }
    ]
  },
  STUDENT: {
    badge: {
      icon: <Sparkles size={14} className='fill-indigo-400 text-indigo-400' />,
    ...
    ctas: [
      { label: 'Find a Tutor', href: '/tutors', icon: <Search size={18} />, variant: 'primary' },
      { label: 'My Dashboard', href: '/dashboard', icon: <LayoutDashboard size={18} />, variant: 'outline' }
    ]
  },
  TUTOR: {
    badge: {
    ...
    ctas: [
      { label: 'My Bookings', href: '/tutor/bookings', icon: <CalendarDays size={18} />, variant: 'primary' },
      { label: 'My Dashboard', href: '/tutor/dashboard', icon: <LayoutDashboard size={18} />, variant: 'outline' }
    ]
  },
  ADMIN: {
    badge: {
    ...
    ctas: [
      { label: 'Admin Dashboard', href: '/admin/dashboard', icon: <Shield size={18} />, variant: 'primary' },
      { label: 'Manage Users', href: '/admin/users', icon: <Users size={18} />, variant: 'outline' }
    ]
  }
}
          ...
          <div className='mt-10 flex flex-col items-center gap-4 sm:flex-row sm:justify-center'>
            {content.ctas.map((cta) => (
              <Link key={cta.href} href={cta.href}>
                <Button variant={cta.variant ?? 'primary'} size='lg' className='gap-2'>
                  {cta.icon}
                  {cta.label}
                </Button>
              </Link>
