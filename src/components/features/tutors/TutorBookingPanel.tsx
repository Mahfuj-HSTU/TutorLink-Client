...
export default function TutorBookingPanel({ tutor }: Props) {
  ...
  // Block booking if student already has an active session with this tutor
  ...
  const availableStartSlots = useMemo(() => {
    ...
    // Apply today's prep-time cutoff
    if (startDate === today) {
      const now = new Date();
    ...
    // Disable slots that fall inside an existing confirmed booking
  ...
  const availableEndSlots = useMemo(() => {
    ...
    // Max end = start of the earliest booking that ends after our start (to avoid any overlap)
  ...
  const handleStartSlotChange = (slot: string) => {
    ...
    if (defaultEndMins < 24 * 60) {
      ...
      setEndSlot(
        ALL_SLOTS.find((s) => toMinutes(s) === defaultEndMins) ?? ""
      );
    } else {
      setEndDate(addDays(startDate, 1));
      const overflowMins = defaultEndMins - 24 * 60;
      setEndSlot(
        ALL_SLOTS.find((s) => toMinutes(s) === overflowMins) ?? ""
      );
    }
  };
      ...
      {user?.role === "STUDENT" && tutor.isAvailable && (
        <div className="relative inline-block group">
          ...
          {hasActiveSameTutorSession && (
            ...
            >
              You have an active session with this tutor. Book again after it&apos;s completed or cancelled.
